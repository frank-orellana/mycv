...
export default class i18n {
    ...
    static selectLang(plang) {
        var lang = plang || window.navigator.languages[0];
        console.log(lang);
        if (i18n.langs[lang])
    ...
    static getMessage(messageId) {
        if (!i18n.msg)
            return;
        let res = i18n.msg[messageId];
        if (!res) {
            let errMsg = `I18N: message "${messageId}" not found in language ${i18n.currentLang}`;
